...
fn main() {
    ...
    match &cil.command {
        Some(options::Commands::Date { year, month }) => {
            let year = year.as_ref().unwrap().parse::<i32>().expect("Error year");
            let month = month.as_ref().unwrap().parse::<u32>().expect("Error month");
        ...
        None => {
            options::Cil::parse_from(["rust-calendar", "help"].iter());
        }
    }
}
